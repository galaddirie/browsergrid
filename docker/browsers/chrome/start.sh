#!/bin/bash
# Update docker/browsers/chrome/start.sh
...
start_chrome() {
  ...
  echo "Starting Chrome with data directory: ${DATA_DIR}"
  ...
  if [ ! -w "${DATA_DIR}" ]; then
    echo "WARNING: Cannot write to data directory ${DATA_DIR}"
    echo "Attempting to fix permissions..."
    
    # Try to fix permissions if possible
    if sudo chmod 755 "${DATA_DIR}" 2>/dev/null; then
  ...
  ls -la "${DATA_DIR}" || echo "Cannot list directory contents"
